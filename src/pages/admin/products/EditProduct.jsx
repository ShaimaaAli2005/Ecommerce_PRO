...
export const EditProduct = () => {
  ...
  // جلب التصنيفات وبيانات المنتج مع Fallback حاسم
    ...
    try {
      ...
      try {
        ...
        const catList =
          ...
          catsRes.data?.data ||
          (Array.isArray(catsRes.data) ? catsRes.data : []);
        setCategories(catList);
      } catch (catErr) {
        console.warn("Categories fetch warning:", catErr);
      }

      // 2. محاولة جلب المنتج المفرد
      let p = null;
      try {
        ...
        // إذا فشل جلب المنتج المفرد بـ 404، نجلبه من القائمة الشاملة
        const allRes = await api.get("/products");
        const list =
          allRes.data?.products ||
          allRes.data?.data ||
          (Array.isArray(allRes.data) ? allRes.data : []);
        p = list.find((item) => String(item._id) === String(id));
      }

      if (!p) {
        throw new Error(t("admin.edit_product.toast.not_found"));
      }

      // استخراج الصور وتوحيدها
      let extractedImages = [];
      if (Array.isArray(p.images) && p.images.length > 0) {
        extractedImages = p.images
          .map((img) => (typeof img === "string" ? img : img?.url || img?.secure_url || ""))
          .filter(Boolean);
      } else if (p.image) {
        extractedImages = [
          typeof p.image === "string" ? p.image : p.image?.url || p.image?.secure_url || "",
        ].filter(Boolean);
      }
      ...
      setError(err.message || t("admin.edit_product.toast.error"));
    } finally {
      setLoading(false);
    }
  }, [id, t]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanImages = formData.images.filter((url) => typeof url === "string" && url.trim().length > 0);
      const parsedTags = formData.tags
        ? formData.tags.split(",").map((t) => t.trim()).filter(Boolean)
        : [];

      // تنظيف الـ Payload بدقة
      const payload = {
      ...
      // محاولة PUT أولاً ثم PATCH كـ Fallback
      try {
      ...
      setSuccessMsg(t("admin.edit_product.toast.success"));
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            >
              ...
              <span>{t("admin.edit_product.back_to_list")}</span>
            </Link>
          ...
          >
            {submitting ? (
              ...
              <>
                ...
                <span>{t("admin.edit_product.submit_btn")}</span>
              </>
            )}
          </button>
        </form>

        {/* ─── المعاينة الحية الفاخرة ─── */}
